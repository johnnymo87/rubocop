...
== Layout/MultilineChainedMethodCallLineBreak
...
This cop mandates that in a method chain, if the initial method call extends over multiple lines due to its arguments or an accompanying block, then subsequent methods must commence on new lines, thereby enhancing readability and providing clear demarcation of complex multiline expressions.
